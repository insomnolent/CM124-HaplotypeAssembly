            ...
            for l in range(0, len(topreads)-1):
                ...
                if overlap(recentread1, read1, i):
                    ...
                    for k in range(l+1, len(topreads)):
                        read2 = topreads[k]
                        if checkInverse(removeDash(read1), removeDash(read2)) and overlap(recentread2, read2, i):
                            new.append(read2)
                            recentread2 = read2
                            break

                if overlap(recentread2, read1, i):
                    new.append(read1)
                    recentread2 = read1
                    for k in range(l + 1, len(topreads)):
                        read2 = topreads[k]
                        if checkInverse(removeDash(read1), removeDash(read2)) and overlap(recentread1, read2, i):
                            new.append(read2)
                            recentread1 = read2
                            break
    return new
